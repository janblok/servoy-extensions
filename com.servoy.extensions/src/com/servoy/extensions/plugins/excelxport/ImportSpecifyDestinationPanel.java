...
public class ImportSpecifyDestinationPanel extends JPanel implements IWizardPanel, ItemListener
{
	...
	protected ArrayList columnsData = new ArrayList();
	...
	public ImportSpecifyDestinationPanel(IWizard parent, IWizardState state, IApplication app) throws RemoteException, RepositoryException
	{
		this.parent = parent;
		this.state = state;
		...
		tableCombobox.setModel(new SortedComboModel(StringComparator.INSTANCE, server == null ? Collections.EMPTY_LIST : server.getTableAndViewNames()));
	...
	public Runnable needsToRunFirst(final boolean forward)
	{
		return new Runnable()
		{
			public void run()
			{
				try
				{
					parent.blockGUI(Messages.getString("servoy.plugin.import.status.organizingDataIntoColumns")); //$NON-NLS-1$
					...
					outer : for (int i = 0; i < columnsModel.getRowCount(); i++)
					{
						...
						{
							if (columns.getItemAt(j).toString().equalsIgnoreCase(importColumn))
							{
	...
	public void itemStateChanged(ItemEvent e)
	{
		if (e.getStateChange() == ItemEvent.SELECTED)
		{
			if (e.getSource() == serverCombobox)
			{
				...
				try
				{
					tableCombobox.setModel(new SortedComboModel(StringComparator.INSTANCE, server.getTableAndViewNames()));
				}
				catch (Exception e1)
				{
					Debug.error(e1);
